...
class SelectAreaScreen extends StatefulWidget { // <<<--- ZMIENIONA NAZWA KLASY
  final Project project;

  const SelectAreaScreen({ // <<<--- ZMIENIONA NAZWA KLASY
    super.key,
    required this.project,
  });

  @override
  _SelectAreaScreenState createState() => _SelectAreaScreenState(); // <<<--- ZMIENIONA NAZWA KLASY STANU
}

class _SelectAreaScreenState extends State<SelectAreaScreen> { // <<<--- ZMIENIONA NAZWA KLASY STANU
  List<Area> _areas = [];
  ...
  Future<void> _getAreas() async {
    ...
    try {
      ...
      if (mounted) {
        final errorMessageText = 'Nie udało się załadować obszarów: ${e.toString()}';
        setState(() {
          _isLoading = false;
  ...
  void _selectAreaAndPop(Area area) {
    // Zwróć wybrany obiekt Area do poprzedniego ekranu
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Naciśnij przycisk "+" aby dodać nowy obszar dla projektu "${widget.project.name}".',
                  textAlign: TextAlign.center,
